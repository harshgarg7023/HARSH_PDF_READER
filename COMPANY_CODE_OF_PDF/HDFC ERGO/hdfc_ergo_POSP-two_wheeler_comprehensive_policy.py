...
patterns = {
    ...
    'Name': re.compile(r'^\s*(?:Mr|Ms|Mrs|Dr|Prof)\s+(.*)$', re.MULTILINE),
    'Contact No.':re.compile(r'Mob\.\s+(\d{10,15})'),
    'Policy Number': re.compile(r'Policy\s+No\.\s+(\d+)'),
    ...
    'Model': re.compile(r'Model\s*-\s*Variant\s*(.+)'),
    ...
    'Total-Add on':re.compile(r'Total-Addon\s+(\d+)'),
    ...
    'Basic Third Party Liability':re.compile(r'Basic Third Party Liability:\s+(\d+)'),
...
def extract_details(text, patterns):
    ...
    for key, pattern in patterns.items():
        ...
        if match:
            ...
            if match.groups():
                details[key] = match.group(1)  # Access the first capture group if it exists
            else:
                details[key] = match.group(0)  # Otherwise, get the entire match
        else:
            details[key] = None
    return details

# Path to the uploaded PDF file
pdf_path = r'C:\Users\user\PycharmProjects\PDF_READER\PDF CONTAINER\HDFC ERGO\2301101447947700000.pdf'
