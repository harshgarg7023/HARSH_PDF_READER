...
patterns = {
    ...
    'Third Party Baisc Premium': re.compile(r'Basic Own Damage:\s*(\d+)'),
    'Total-Add on':re.compile(r'Total - Add on\s+(\d+)'),
    ...
    'Total TP': re.compile(r'Total Premium\s+([\d,.]+)'),
    'Total Package Premium (a+b)': re.compile(r'Total Premium\s+([\d,.]+)'),
...
pdf_path = r'C:\Users\user\PycharmProjects\PDF_READER\PDF CONTAINER\HDFC ERGO\2302101448366100000.pdf'
...
try:
    total_package_premium = float(details.get('Total Package Premium (a+b)', '0').replace(',', ''))
    gst = float(details.get('GST', '0').replace(',', ''))
    final_premium = total_package_premium + gst
except ValueError:
    final_premium = None

# Print the final premium in the desired format
if final_premium is not None:
    print(f"Final Premium = {final_premium}")
else:
    print("Error calculating final premium.")