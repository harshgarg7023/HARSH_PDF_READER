...
pdf_file_path = r"C:\Users\user\PycharmProjects\PDF_READER\PDF CONTAINER\CHOLAMS\33616046064900000.pdf"
...
patterns = {
    'policy_type': r'Policy Schedule cum Certificate of Insurance\s+([^\s]+(?: [^\s]+){2,})\s+UIN',
    'policy_number': r'Policy cum Certificate Number\s*([0-9A-Z/]+)',
    'insured_name': r'Name & Communication Address:\s*([A-Za-z\s](.*)+)',
    'customer_phone_number': r'Mobile/Landline No\s*:\s*([\d\s-]+)',
    'insured_address': r'Registration Address:\s*([\s\S]*?\d{6})',
    ...
    'gst': r'IGST \(18%\)\s*Rs\.\s*([\d]+)',
    # 'final_premium': r'TOTAL AMOUNT\s*Rs\.\s*([\d]+)',
    'final_premium': r"TOTAL AMOUNT\s*Rs\.\s*([\d,\.]+)",
